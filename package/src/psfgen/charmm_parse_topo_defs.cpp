...
int charmm_parse_topo_defs(topo_defs *defs, FILE *file, int all_caps) {
	...
	while ( (ntok = charmm_get_tokens(tok,TOKLEN,sbuf,BUFLEN,file,all_caps)) ) {
		...
		if ( skipall ) {
			PSFGEN_INFO("skipping statements at end of file due to end or return statement");
			break;
		}
		if ( first ) {
			first = 0;
			if ( ! strncasecmp("READ",tok[0],4) && ! strncasecmp("RTF",tok[1],4) ) {
				PSFGEN_INFO("reading topology from stream file");
				first = 1;
				stream = 1;
				continue;
			} else if ( ! strncasecmp("READ",tok[0],4) && ! strncasecmp("PARA",tok[1],4) ) {
				PSFGEN_INFO("skipping parameters in stream file");
				skip = 1;
				continue;
			} else if ( ! strncasecmp("READ",tok[0],4) ) {
				PSFGEN_INFO("skipping unknown section in stream file");
				skip = 1;
				continue;
		...
		if ( skip ) {
			if ( ! strncasecmp("END",tok[0],4) ) {
				PSFGEN_DEBUG("Recognized file end statement in skipped section.");
				skip = 0;
				first = 1;
			}
		}
		else if ( ! strncasecmp("END",tok[0],4) ) {
			PSFGEN_DEBUG("Recognized file end statement.");
			if ( stream ) {
				stream = 0;
				first = 1;
			} else {
			...
			PSFGEN_DEBUG("Recognized return statement.");
			skipall = 1;
		}
		else if ( ! strncasecmp("ACCE",tok[0],4) ) {
			PSFGEN_DEBUG("Recognized acceptor statement.");
		}
		else if ( ! strncasecmp("DONO",tok[0],4) ) {
			PSFGEN_DEBUG("Recognized donor statement.");
		}
		else if ( ! strncasecmp("BOND",tok[0],4) ||
				! strncasecmp("DOUB",tok[0],4) ||
				! strncasecmp("TRIP",tok[0],4) ) {
			PSFGEN_DEBUG("Recognized bond statement.");
			if ( ntok < 3 || (ntok-1)%2 ) {
				PSFGEN_ERROR("Failed to parse bond statement.");
			} else {
			...
			PSFGEN_DEBUG("Recognized angle statement.");
			if ( ntok < 4 || (ntok-1)%3 ) {
				PSFGEN_ERROR("Failed to parse angle statement.");
			} else {
			...
			PSFGEN_DEBUG("Recognized dihedral statement.");
			if ( ntok < 5 || (ntok-1)%4 ) {
				PSFGEN_ERROR("Failed to parse dihedral statement.");
			} else {
			...
			PSFGEN_DEBUG("Recognized improper statement.");
			if ( ntok < 5 || (ntok-1)%4 ) {
				PSFGEN_ERROR("Failed to parse improper statement.");
			} else {
				for ( itok = 1; itok < ntok; itok += 4 ) {
					s1 = parse_atom(tok[itok],&i1,&j1);
					s2 = parse_atom(tok[itok+1],&i2,&j2);
			...
			PSFGEN_DEBUG("Recognized CMAP statement.");
			if ( ntok != 9 ) {
				PSFGEN_ERROR("Failed to parse CMAP statement.");
			} else {
			...
			PSFGEN_DEBUG("Recognized atom declaration statement.");
		}
		else if ( ! strncasecmp("ATOM",tok[0],4) ) {
			PSFGEN_DEBUG("Recognized atom statement.");
			if ( ntok < 4 ) {
				PSFGEN_ERROR("Failed to parse atom statement.");
			} else if ( ntok > 4 ) {
				PSFGEN_ERROR("Explicit exclusions or fluctuating charges not supported, atom ignored.");
			} else {
			...
			PSFGEN_DEBUG("Recognized mass statement.");
			if ( ntok < 4 || topo_defs_type(defs,tok[2],(ntok>4?tok[4]:""),atof(tok[3]),atoi(tok[1])) ) {
				PSFGEN_ERROR("Failed to parse mass statement.");
			}
		}
		else if ( ! strncasecmp("AUTO",tok[0],4) ) {
			PSFGEN_DEBUG("Recognized autogenerate statement.");
			for ( itok = 1; itok < ntok; itok += 1 ) {
				if ( ! strncasecmp("ANGL",tok[itok],4) ) {
					...
					PSFGEN_ERROR("Failed to parse autogenerate statement.");
				}
			}
		}
		else if ( ! strncasecmp("DEFA",tok[0],4) ) {
			PSFGEN_DEBUG("Recognized default patch statement.");
			if ( ntok < 3 || (ntok-1)%2 ) {
				PSFGEN_ERROR("Failed to parse default patching statement.");
			} else {
				i1 = i2 = 0;
				for ( itok = 1; itok < ntok; itok += 2 ) {
			...
			PSFGEN_DEBUG("Recognized internal coordinate statement.");
			if ( ntok < 10 ) {
				PSFGEN_ERROR("Failed to parse internal coordinate statement.");
			} else {
				s1 = parse_atom(tok[1],&i1,&j1);
			...
			PSFGEN_DEBUG("Recognized delete statement.");
			if ( ntok < 2 ) {
				PSFGEN_ERROR("Failed to parse delete statement.");
			} else {
				if ( ! strncasecmp("ATOM",tok[1],4) ) {
					if ( ntok < 3 ) {
						PSFGEN_ERROR("Failed to parse delete atom statement.");
					} else {
					...
					if ( ntok < 4 || (ntok-2)%2 ) {
						PSFGEN_ERROR("Failed to parse delete bond statement.");
					} else {
					...
					if ( ntok < 5 || (ntok-2)%3 ) {
						PSFGEN_ERROR("Failed to parse delete angle statement.");
					} else {
					...
					if ( ntok < 6 || (ntok-2)%4 ) {
						PSFGEN_ERROR("Failed to parse delete dihedral statement.");
					} else {
					...
					if ( ntok < 6 || (ntok-2)%4 ) {
						PSFGEN_ERROR("Failed to parse delete improper statement.");
					} else {
						for ( itok = 2; itok < ntok; itok += 4 ) {
							s1 = parse_atom(tok[itok],&i1,&j1);
							s2 = parse_atom(tok[itok+1],&i2,&j2);
					...
					if ( ntok < 6 ) {
						PSFGEN_ERROR("Failed to parse delete internal coordinate statement.");
					} else {
						s1 = parse_atom(tok[2],&i1,&j1);
			...
			PSFGEN_DEBUG("Recognized group statement.");
		}
		else if ( ! strncasecmp("PATC",tok[0],4) ) {
			PSFGEN_DEBUG("Recognized patching statement.");
			if ( ntok < 3 || (ntok-1)%2 ) {
				PSFGEN_ERROR("Failed to parse patching statement.");
			} else {
				i1 = i2 = 0;
				for ( itok = 1; itok < ntok; itok += 2 ) {
					if ( ! strncasecmp("FIRS",tok[itok],4) ) {
			...
			PSFGEN_DEBUG("Recognized residue statement.");
			if ( ntok < 2 || defs->build_residue(tok[1],0) ) {
				PSFGEN_ERROR("Failed to parse residue statement.");
			}
		}
		else if ( ! strncasecmp("PRES",tok[0],4) ) {
			PSFGEN_DEBUG("Recognized patch residue statement.");
			if ( ntok < 2 || defs->build_residue(tok[1],1) ) {
				PSFGEN_ERROR("Failed to parse patch residue statement.");
			}
		}
		else {
