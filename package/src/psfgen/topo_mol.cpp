...
static int topo_mol_patch(topo_mol *mol, const topo_mol_ident_t *targets,
                          int ntargets, const char *rname, int prepend,
                          int warn_angles, int warn_dihedrals, int deflt);


static std::string strtoupper(const char* str, bool all_caps) {
    std::string res = str;
    if (all_caps) {
...
static psfgen_logging_level logging_level = LOG_INFO;
static const char *logging_levels[] = {"DEBUG", "INFO", "WARN", "ERROR", "CRITICAL"};

extern "C" void psfgen_log(psfgen_logging_level lvl, const char* msg, const char* src) {

    const char* str;
    if(lvl < LOG_DEBUG || lvl > LOG_CRITICAL) {
        str = "ERROR, INVALID LOGGING LEVEL";
        lvl = LOG_ERROR;
    } else {
        str = logging_levels[lvl];
    } 
...
extern "C" void psfgen_msg(void* v, const char* s) {
    psfgen_log(LOG_INFO, s, __PRETTY_FUNCTION__);
}

topo_mol * topo_mol_create(topo_defs *defs) {
    return new topo_mol(defs);
}
...
static topo_mol_atom * topo_mol_get_atom(topo_mol *mol,
		const topo_mol_ident_t *target, int irel) {
	topo_mol_residue_t *res;
	topo_mol_atom *atom;
	char errmsg[64 + 3*NAMEMAXLEN];
...
static topo_mol_atom *topo_mol_get_atom_from_res(
...
int topo_mol_segment_auto_angles(topo_mol *mol, int autogen) {
	if ( ! mol ) return -1;
	if ( ! mol->buildseg ) {
		PSFGEN_ERROR("no segment in progress for auto angles");
		return -1;
	}
...
int topo_mol_segment_auto_dihedrals(topo_mol *mol, int autogen) {
	if ( ! mol ) return -1;
	if ( ! mol->buildseg ) {
		PSFGEN_ERROR("no segment in progress for auto dihedrals");
		return -1;
	}
...
int topo_mol_residue(topo_mol *mol, const char *resid, const char *rname,
	...
	if ( ! mol ) return -1;
	if ( ! mol->buildseg ) {
		PSFGEN_ERROR("no segment in progress for residue");
		return -1;
	}
	seg = mol->buildseg;
...
int topo_mol_mutate(topo_mol *mol, const char *resid, const char *rname) {
	...
	if ( ! mol ) return -1;
	if ( ! mol->buildseg ) {
		PSFGEN_ERROR("no segment in progress for mutate");
		return -1;
	}
	seg = mol->buildseg;
...
static topo_mol_atom * topo_mol_unlink_atom(
		topo_mol_atom **atoms, const char *aname) {
	topo_mol_atom **atom;
	topo_mol_atom *oldatom;
	if ( ! atoms ) return 0;
...
static topo_mol_atom * topo_mol_find_atom(
		topo_mol_atom **atoms, const char *aname) {
	topo_mol_atom **atom;
...
static int topo_mol_add_atom(topo_mol *mol, topo_mol_atom **atoms,
		topo_mol_atom **oldatoms, topo_defs_atom_t *atomdef) {
	int idef;
	topo_mol_atom *atomtmp;
	...
	if ( ! atomtmp ) {
		atomtmp = (topo_mol_atom*)memarena_alloc(mol->arena,sizeof(topo_mol_atom));
		if ( ! atomtmp ) return -2;
...
topo_mol_chargegroup * topo_mol_chargegroup_next(
		topo_mol_chargegroup *group, topo_mol_atom *atom) {
...
static topo_mol_conformation * topo_mol_conformation_next(
		topo_mol_conformation *tuple, topo_mol_atom *atom) {
	if ( tuple->atom[0] == atom ) return tuple->next[0];
	if ( tuple->atom[1] == atom ) return tuple->next[1];
	if ( tuple->atom[2] == atom ) return tuple->next[2];
	if ( tuple->atom[3] == atom ) return tuple->next[3];
...
static void topo_mol_destroy_atom(topo_mol_atom *atom) {
	topo_mol_bond *bondtmp;
	topo_mol_angle *angletmp;
	topo_mol_dihedral *dihetmp;
	topo_mol_improper *imprtmp;
	topo_mol_cmap *cmaptmp;
	topo_mol_conformation *conftmp;
	if ( ! atom ) return;
...
static int add_bond_to_residues(topo_mol *mol, 
	...
	if (!a1 || !a2) return -1;
	tuple = (topo_mol_bond*)memarena_alloc(mol->arena,sizeof(topo_mol_bond));
	if ( ! tuple ) return -10;
...
static int topo_mol_add_angle(topo_mol *mol, const topo_mol_ident_t *targets,
	...
	topo_mol_angle *tuple;
	topo_mol_atom *a1, *a2, *a3;
	...
	if ( ! a3 ) return -7;
	tuple = (topo_mol_angle*)memarena_alloc(mol->arena,sizeof(topo_mol_angle));
	if ( ! tuple ) return -10;
...
static int topo_mol_add_dihedral(topo_mol *mol, const topo_mol_ident_t *targets,
	...
	topo_mol_dihedral *tuple;
	topo_mol_atom *a1, *a2, *a3, *a4;
	...
	if ( ! a4 ) return -9;
	tuple = (topo_mol_dihedral*)memarena_alloc(mol->arena,sizeof(topo_mol_dihedral));
	if ( ! tuple ) return -10;
...
static int add_improper_to_residues(topo_mol *mol, 
	...
	tuple = (topo_mol_improper*)memarena_alloc(mol->arena,sizeof(topo_mol_improper));
	if ( ! tuple ) return -10;
...
static int add_cmap_to_residues(topo_mol *mol, 
	...
	tuple = (topo_mol_cmap*)memarena_alloc(mol->arena,sizeof(topo_mol_cmap));
	if ( ! tuple ) return -20;
	for ( i=0; i<8; ++i ) {
...
static int add_conformation_to_residues(topo_mol *mol, 
	...
	a4 = topo_mol_get_atom_from_res(res4, aname4);
	if (!a1 || !a2 || !a3 || !a4) return -1;
	tuple = (topo_mol_conformation*)memarena_alloc(mol->arena,sizeof(topo_mol_conformation));
	if ( ! tuple ) return -10;
...
int topo_mol_end(topo_mol *mol) {
	...
	if ( ! mol ) return -1;
	if ( ! mol->buildseg ) {
		PSFGEN_ERROR("no segment in progress for end");
		return -1;
	}
	seg = mol->buildseg;
	mol->buildseg = 0;
...
static int topo_mol_auto_dihedrals(topo_mol *mol, topo_mol_segment_t *segp) {
	...
	topo_mol_segment_t *seg;
	topo_mol_residue_t *res;
	topo_mol_angle *g1, *g2;
	topo_mol_dihedral *tuple;
	topo_mol_atom *atom, *a1=0, *a2=0, *a3=0, *a4=0;
...
int topo_mol_patch(topo_mol *mol, const topo_mol_ident_t *targets,
	...
	if ( ! mol ) {
		PSFGEN_ERROR("mol is NULL");
		return -1;
	}
	if ( mol->buildseg ) {
		PSFGEN_ERROR("Currently building a segment, patches may not be applied whilst building a segment.");
		return -2;
	}
	if ( ! mol->defs ) {
		PSFGEN_ERROR("No definitions found, topo_mol::defs is NULL");
		return -3;
	}
...
int topo_mol_multiply_atoms(topo_mol *mol, const topo_mol_ident_t *targets,
	...
	for (ipass=0; ipass<2; ++ipass) {
		if ( ipass ) atoms = (topo_mol_atom**)memarena_alloc(mol->arena,
				natoms*sizeof(topo_mol_atom*));
		natoms = 0;
		/* walk all targets */
	...
	for (icopy=1; icopy<ncopies; ++icopy) {
		...
		for (iatom=0; iatom<natoms; ++iatom) {
			topo_mol_atom *newatom;
			atom = atoms[iatom];
			if ( atom->copy ) {
				PSFGEN_ERROR("an atom occurs twice in the selection");
				return -20;
			}
			newatom = (topo_mol_atom*)memarena_alloc(mol->arena,sizeof(topo_mol_atom));
			if ( ! newatom ) return -5;
			memcpy(newatom,atom,sizeof(topo_mol_atom));
			atom->next = newatom;
			atom->copy = newatom;
			newatom->bonds = 0;
			newatom->angles = 0;
...
void topo_mol_delete_atom(topo_mol *mol, const topo_mol_ident_t *target) {
	...
	if (!target->resid) {
		...
		if (hasharray_delete(mol->segment_hash, target->segid) < 0) {
			PSFGEN_ERROR( "Unable to delete segment");
		}
		return;
	}
...
void topo_mol::delete_atom(topo_mol *mol, const topo_mol_ident_t *target) {
    return topo_mol_delete_atom(this, target);
}

std::vector<topo_mol_atom*> topo_mol::select_atom(const topo_mol_ident_t *target) {
    topo_mol *mol = this;
    std::vector<topo_mol_atom*> list;
    ...
    {
        std::stringstream ss;
        ss << "{segid:" << (target->segid ? target->segid : "NULL");
        ss << ",resid:" << (target->resid ? target->resid : "NULL");
        ss << ",aname:" << (target->aname ? target->aname : "NULL") << "}";
	...
	if (!target->resid || !strlen(target->resid)) {
		// grab all atoms from this segment
		int nres = hasharray_count(seg->residue_hash);
		for ( ires=0; ires<nres; ++ires ) {
			topo_mol_atom *atom;
			res = &(seg->residue_array[ires]);
	...
	/* Just delete one atom */
    topo_mol_atom *atom = topo_mol_find_atom(&(res->atoms),target->aname);
    list.push_back(atom);
...
int topo_mol_guess_xyz(topo_mol *mol) {
	...
	topo_mol_atom **uatoms;
	topo_mol_conformation *conf;
	...
	uatoms = (topo_mol_atom**) malloc(ucount*sizeof(topo_mol_atom*));
	if ( ! uatoms ) return -2;
	ucount = 0;
	nseg = hasharray_count(mol->segment_hash);
	for ( iseg=0; iseg<nseg; ++iseg ) {
...
int topo_mol_add_patch(topo_mol *mol, const char *pname, int deflt) {
	...
	patchtmp = 0;
	patchtmp = (topo_mol_patch_t*)memarena_alloc(mol->arena,sizeof(topo_mol_patch_t));
...
/* Copied and modified from topo_mol_residue_t */
int topo_mol_add_patchres(topo_mol *mol, const topo_mol_ident_t *target) {
	...
	patchrestmp = 0;
	patchrestmp = (topo_mol_patchres_t*)memarena_alloc(mol->arena,sizeof(topo_mol_patchres_t));
...
void topo_mol::delete_bonds(const topo_mol_ident_t *a, const topo_mol_ident_t *b) {

}


topo_mol::topo_mol() {
	PSFGEN_DEBUG("default ctor");
	topo_defs *defs = topo_defs_create();
    _topo_mol(defs);
}

topo_mol::topo_mol(topo_defs *defs) {
	PSFGEN_DEBUG("defs ctor");
    _topo_mol(defs);
}

void topo_mol::_topo_mol(topo_defs *defs) {

	if ( ! defs ) {
        // TODO error return 0;
        throw (std::runtime_error("could not create a topo_mol with NULL defs"));
    }	
    ...
    this->curpatch = 0;
    this->all_caps = true;
    this->segment_hash = hasharray_create(
        (void**) &(this->segment_array), sizeof(topo_mol_segment_t*));
    this->buildseg = 0;
    this->arena = memarena_create();
    if ( ! this->segment_hash || ! this->arena ) {
...
topo_mol::~topo_mol() {
    PSFGEN_DEBUG("dtor");
    topo_defs_destroy(this->defs);
    this->defs = 0;

    int i,n;
	topo_mol_segment_t *s;

	n = hasharray_count(this->segment_hash);
	for ( i=0; i<n; ++i ) {
		s = this->segment_array[i];
		if ( ! s ) continue;
		hasharray_destroy(s->residue_hash);
	}
	hasharray_destroy(this->segment_hash);
	memarena_destroy(this->arena);
}


int topo_mol::read_topology(const char* filename) {
    FILE *defs_file = 0;
    ...
    if ( ! ( defs_file = fopen(filename,"r") ) ) {
        ...
        charmm_parse_topo_defs(this->defs, defs_file, this->all_caps);
        topo_defs_add_topofile(this->defs, filename);
        fclose(defs_file);
        return 0;
    }
}

int topo_mol::read_psf(const char* filename) {
	FILE *psf_file;
    int retval;
    
    std::string msg = "attempting to read psf file \"";
    msg += filename;
    msg += "\"";
    PSFGEN_DEBUG(msg.c_str());
    ...
    if ( ! ( psf_file = fopen(filename,"rb") ) ) {
        ...
        msg = "reading structure from psf file \"";
        msg += filename;
        msg += "\"";
        PSFGEN_INFO(msg.c_str());

        retval = psf_file_extract(this, psf_file, 0, psfgen_msg);
        fclose(psf_file);
    }
    return retval;
}
...
int topo_mol::num_patch_targets(const char *presname) { 
    topo_defs_residue *resdef;
    topo_defs_atom_t *atomdef;
    topo_defs_bond_t *bonddef;
    ...
    {
        std::string pres = strtoupper(presname, this->all_caps);
        idef = hasharray_index(defs->residue_hash, pres.c_str());
    }
    if (idef == HASHARRAY_FAIL) {
        printf("No such patch residue: \'%s\'\n", presname);
        // TODO error
        return -1;
    }
...
int topo_mol::patch(const char *patch_res_name, PyObject *dict,
                    int prepend, int warn_angles, int warn_dihedrals, int deflt) {

	topo_mol_ident_t target = {0,0,0};
	int res = -1;
	if((res = ident_from_dict(dict, &target)) == 0) {
		return topo_mol_patch(this, &target, 1, patch_res_name, prepend,warn_angles,warn_dihedrals,deflt);
	} else {
		PSFGEN_ERROR("Could not extract target information from dict, expected a dictionary");
		return res;
	}
}


int topo_mol::build_segment(const char *segid) {
    return topo_mol_segment(this, segid);
}

int topo_mol::build_residue(const char *resid, const char *rname, const char *chain) {
    return topo_mol_residue(this, resid, rname, chain);
}


int topo_mol::end() {
    return topo_mol_end(this);
}


std::vector<topo_mol_segment_t*> topo_mol::get_segments() {
    std::vector<topo_mol_segment_t*> result;
    int n = hasharray_count(segment_hash);
    for(int i = 0; i < n; i++) {
        result.push_back(segment_array[i]);
    }
    return result;
}

std::vector<topo_mol_residue_t*> topo_mol_segment_t::get_residues() {
    std::vector<topo_mol_residue_t*> result;
...
std::vector<topo_mol_atom*> topo_mol_residue_t::get_atoms() {
    std::vector<topo_mol_atom*> result;
    for(topo_mol_atom *p = atoms; p; p = p->next) {
        result.push_back(p);
    }
    return result;
}

std::string topo_mol_atom::__str__() {
    std::stringstream ss;
    ss << "<topo_mol_atom, name:" << namestr(name) << ",";
    ss << "type:" << namestr(type) << ",";
    ss << "element:" << namestr(element) << ",";
    ss << "xyz:[" << x << "," << y << "," << z << "],";
    ss << "id:" << atomid << ">";
    return ss.str();
}

std::string topo_mol_residue_t::__str__() {
    std::stringstream ss;
    ss << "<topo_mol_residue_t, resid:" << namestr(resid) << ",";
    ss << "name:" << namestr(name) << ",";
    ss << "chain:" << namestr(chain) << ">";
    return ss.str();
}

std::string topo_mol_segment_t::__str__() {
    std::stringstream ss;
    ss << "<topo_mol_segment_t, segid:" << namestr(segid) << ",";
    ss << "pfirst:" << namestr(pfirst) << ",";
    ss << "plast:" << namestr(plast) << ">";
    return ss.str();
}


int topo_mol::read_residues(const char* filename) {
    FILE *res_file;

    if ( ! ( res_file = fopen(filename,"r") ) ) {
        std::string msg = "Unable to open pdb file %s to read residues";
        msg += filename;
        PSFGEN_ERROR(msg.c_str());
        return -1;
    } else {
        std::string msg = "reading residues from pdb file ";
        msg += filename;
        PSFGEN_INFO(msg.c_str());
        if ( pdb_file_extract_residues(this, res_file, aliases,
                                       all_caps) ) {
            msg = "failed on reading residues from pdb file";
            fclose(res_file);
            PSFGEN_ERROR(msg.c_str());
            return -1;
        }
...
int topo_mol::read_coordinates(const char* filename, const char* segid) {
    FILE *res_file;
    std::string segidstr;
    int rc;
    ...
    if ( ! ( res_file = fopen(filename,"r") ) ) {
        ...
        msg += filename;
        msg += " to read coordinates";
        PSFGEN_ERROR(msg.c_str());
        return -1;
    } else {
        if (segid && strlen(segid)) {
            /* Read only coordinates for given segid */
            segidstr = strtoupper(segid, all_caps);

            std::string msg = "reading coordinates from pdb file ";
            msg += filename;
            msg += " for segment ";
            msg += segidstr;
            PSFGEN_INFO(msg.c_str());
        ...
        const char* psegid = segidstr.size() ? segidstr.c_str() : 0;
        rc=pdb_file_extract_coordinates(this,res_file,psegid,aliases,all_caps);
    
        if (rc) {
            std::string msg = "failed on reading coordinates from pdb file";
            fclose(res_file);
            PSFGEN_ERROR(msg.c_str());
            return -1;
        }
...
int topo_mol::mutate(const char *resid, const char *rname) {
...
int topo_mol::multiply_atoms(const topo_mol_ident_t *target, int ncopies){
    return topo_mol_multiply_atoms(this, target, 1, ncopies);
}

int topo_mol::set_element(const topo_mol_ident_t *target,
                const char *element, int replace) {
    return topo_mol_set_element(this, target, element, replace);
}

int topo_mol::set_chain(const topo_mol_ident_t *target,
              const char *chain, int replace) {
    return topo_mol_set_chain(this, target, chain, replace);
}

int topo_mol::set_coordinates(const topo_mol_ident_t *target,
                    double x, double y, double z) {
    return topo_mol_set_xyz(this, target, x, y, z);
}


int topo_mol::guess_coordinates() {
    return topo_mol_guess_xyz(this);
}

int topo_mol::add_patch(const char *pname, int deflt) {
    return topo_mol_add_patch(this, pname, deflt);
}

int topo_mol::add_patchres(const topo_mol_ident_t *targets) {
    return topo_mol_add_patchres(this, targets);
}

int topo_mol::validate_patchres(const char *pname, const char *segid, const char *resid) {
    return topo_mol_validate_patchres(this, pname, segid, resid);
}

int topo_mol::alias_residue(const char* _altres, const char* _realres) {
    std::string altres=strtoupper(_altres, all_caps);
    std::string realres=strtoupper(_realres, all_caps);
    std::string msg = "aliasing residue ";
    msg += _altres;
    msg += " to ";
    msg += _realres;
    PSFGEN_INFO(msg.c_str());
    return extract_alias_residue_define(aliases, altres.c_str(), realres.c_str());
}

int topo_mol::alias_atom(const char* _resname, const char* _altatom, const char* _realatom) {
    std::string resname=strtoupper(_resname, all_caps);
    std::string altatom=strtoupper(_altatom, all_caps);
    std::string realatom=strtoupper(_realatom, all_caps);
    std::string msg = "aliasing residue ";
    msg += resname;
    msg += " atom ";
    msg += altatom;
    ...
    msg += realatom;
    PSFGEN_INFO(msg.c_str());
    return extract_alias_atom_define(aliases,resname.c_str(),altatom.c_str(),realatom.c_str());
}
...
int ident_from_dict(PyObject *dict, topo_mol_ident_t *ident) {
	...
	tmpobj = PyDict_GetItemString(dict, "segid");
	ident->segid = tmpobj ? PyString_AsString(tmpobj) : 0;
	tmpobj = PyDict_GetItemString(dict, "resid");
	ident->resid = tmpobj ? PyString_AsString(tmpobj) : 0;
	tmpobj = PyDict_GetItemString(dict, "aname");
	ident->aname = tmpobj ? PyString_AsString(tmpobj) : 0;
...
int logging_set_level(const std::string& lvl) {
	...
	if(strncasecmp(lvl.c_str(), logging_levels[LOG_DEBUG], strlen(logging_levels[LOG_DEBUG])) == 0) {
		logging_level = LOG_DEBUG;
		PSFGEN_INFO("setting logging level to DEBUG");
		result = 0;
	}
	else if(strncasecmp(lvl.c_str(), logging_levels[LOG_INFO], strlen(logging_levels[LOG_INFO])) == 0) {
		logging_level = LOG_INFO;
		PSFGEN_INFO("setting logging level to INFO");
		result = 0;
	}
	else if(strncasecmp(lvl.c_str(), logging_levels[LOG_WARN], strlen(logging_levels[LOG_WARN])) == 0) {
		logging_level = LOG_WARN;
		PSFGEN_INFO("setting logging level to WARN");
		result = 0;
	}
	else if(strncasecmp(lvl.c_str(), logging_levels[LOG_ERROR], strlen(logging_levels[LOG_ERROR])) == 0) {
		logging_level = LOG_ERROR;
		PSFGEN_INFO("setting logging level to ERROR");
		result = 0;
	}
	else if(strncasecmp(lvl.c_str(), logging_levels[LOG_CRITICAL], strlen(logging_levels[LOG_CRITICAL])) == 0) {
		logging_level = LOG_DEBUG;
		PSFGEN_INFO("setting logging level to CRITICAL");
		result = 0;
	}
	else {
		std::stringstream ss;
		ss << "\'" << lvl << "\' is NOT a valid logging level, valid levels are [";
...
std::string logging_get_level() {
	return std::string(logging_levels[logging_level]);
}
...
int set_logger(PyObject *logger) {
	PSFGEN_WARN("NOT IMPLEMENTED YET");
	return -1;
}
