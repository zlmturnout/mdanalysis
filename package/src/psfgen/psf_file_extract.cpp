...
static int extract_chargegroups(FILE *file, topo_mol *mol, int natoms,
    ...
    {
        std::stringstream ss; ss << "nchargegroups: " << nchargegroups;
...
static int extract_angles(FILE *file, topo_mol *mol, int natoms, 
    ...
    for (i=nangles-1; i >= 0; i--) {
        topo_mol_atom *atom1, *atom2, *atom3;
...
static int extract_dihedrals(FILE *file, topo_mol *mol, int natoms,
    ...
    for (i=ndihedrals-1; i >= 0; i--) {
        topo_mol_atom *atom1, *atom2, *atom3, *atom4;
...
int psf_file_extract(topo_mol *mol, FILE *file, void *v,
    ...
    molatomlist = (topo_mol_atom **)malloc(natoms * sizeof(topo_mol_atom *));
    ...
    if (extract_impropers(file, mol, natoms, molatomlist)) {
        const char* msg = " Error processing impropers";
        print_msg(v, msg );
        free(atomlist);
        free(molatomlist);
        return -1;
