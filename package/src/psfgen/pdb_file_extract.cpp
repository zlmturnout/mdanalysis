    ...
    if((indx = read_pdb_record(file, record)) == PDB_ATOM) {
      ...
      if ( strcmp(oldresid,resid) ) {
        ...
        if ( topo_mol_residue(mol,resid,realres,chain) ) {
          sprintf(msg,"failed on residue %s from pdb file",resname);
          PSFGEN_ERROR(msg);
        }
      }
    }
...
int pdb_file_extract_coordinates(topo_mol *mol, FILE *file,
  ...
  do {
    if((indx = read_pdb_record(file, record)) == PDB_ATOM) {
      ...
      if ( ! found ) {
        sprintf(msg,"failed to set coordinate for atom %s\t %s:%s\t  %s",name,resname,resid,segid ? segid : segname);
        PSFGEN_WARN(msg);
      } else {
        /* only try element and chain if coordinates succeeds */
        if ( strlen(element) && topo_mol_set_element(mol,&target,element,0) ) {
          sprintf(msg,"failed to set element for atom %s\t %s:%s\t  %s",name,resname,resid,segid ? segid : segname);
          PSFGEN_WARN(msg);
        }
        if ( strlen(chain) && topo_mol_set_chain(mol,&target,chain,0) ) {
          sprintf(msg,"failed to set chain for atom %s\t %s:%s\t  %s",name,resname,resid,segid ? segid : segname);
          PSFGEN_WARN(msg);
        }
      }
    }
